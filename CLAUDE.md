...
**Next step:** `GET /restaurants/:id` — single restaurant by id. New concepts for this route: Express route params (`:id` in the path), reading `req.params.id`, `prisma.restaurant.findUnique({ where: { id } })`, and **not-found handling** (Prisma returns `null` for a missing id — must send a real 404 instead of `null` with a 200). Do the happy path first, then add the 404 check. After that: a Bruno collection. (Still teaching-mode — hints, not code.)
